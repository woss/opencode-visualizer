...
const TaskSchema = z.object({
  id: z.string().regex(
    /^\d+\.\d+$/,
    "Task ID must be hierarchical (e.g., '2.1')",
  ),
  checked: z.boolean(),
  ...
  if (fmMatch) {
    ...
    for (const line of fmLines) {
      ...
      if (key && valueParts.length > 0) {
        ...
        frontmatter[key.trim()] = key.trim() === "phase"
          ? parseInt(value, 10)
          : value;
      }
    }
...
function formatZodErrors(error: z.ZodError): string {
  ...
  for (const issue of error.issues) {
    ...
    if (issue.code === "invalid_value") {
      ...
      message = `Invalid value "${input}". Expected: ${
        values?.join(" | ") ?? "valid value"
      }`;
    } else if (
...
function parsePlanMarkdown(content: string): ParseResult {
  ...
  if (currentCount > 1) {
    return {
      ok: false,
      error:
        `Multiple tasks marked ← CURRENT (found ${currentCount}). Only one task may be current.`,
      hint: skillHint,
    };
  }
      ...
      plan_save: tool({
        ...
        args: {
          content: tool.schema.string().describe(
            "The full plan in markdown format",
          ),
        },
...
Today is ${today}. When searching for documentation, APIs, or external resources, use the current year (${
        new Date().getFullYear()
      }). Do not default to outdated years from training data.
</date-awareness>`);
