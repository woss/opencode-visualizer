...
export function buildBatchCommandFromArgv(argv?: string[]): string | undefined {
  ...
  return normalizedArgv.map((arg) =>
    `"${escapeBatch(arg).replace(/"/g, '""')}"`
  ).join(" ");
}
...
exec $SHELL`,
      ...
      if (createResult.exitCode !== 0) {
        return {
          success: false,
          error:
            `Failed to create tmux window: ${createResult.stderr.toString()}`,
        };
      }
...
export async function openCmuxTerminalWithState(
  ...
  const runCmuxCommand: RunCmuxCommand = options?.runCmuxCommand ??
  ...
  try {
    ...
    if (currentTerminal) {
      ...
      switch (currentTerminal) {
        ...
        case "warp": {
          const configName = `worktree-${Date.now()}-${
...
export async function openTerminal(
  ...
  if (terminalType === "cmux") {
    const cmuxResult =
      await (options?.openCmuxTerminalWithState ?? openCmuxTerminalWithState)(
        cwd,
        argv,
      );
