...
async function detectTerminalInfo(config: NotifyConfig): Promise<TerminalInfo> {
  ...
  const processName = TERMINAL_PROCESS_NAMES[terminalName.toLowerCase()] ||
    terminalName;
...
function buildQuestionEventDedupeKey(properties: unknown): string | null {
  ...
  const toolInfo = record.tool && typeof record.tool === "object"
...
const NotifyPlugin: Plugin = async (ctx) => {
  ...
  const buildBusySpinnerTitle = (): string => {
    const frame = CMUX_BUSY_ANIMATION_FRAMES[titleBusySpinnerFrameIndex] ??
      CMUX_BUSY_ANIMATION_FRAMES[0];
    titleBusySpinnerFrameIndex = (titleBusySpinnerFrameIndex + 1) %
      CMUX_BUSY_ANIMATION_FRAMES.length;
  ...
  const drainCmuxSessionStatusWrites = async (): Promise<void> => {
    ...
    try {
      while (!cmuxStatusUpdatesDisabled) {
        ...
        const didUpdateStatus = await runCmuxSessionStatusWrite(
          nextWriteIntent,
        );
      ...
      if (
        !cmuxStatusUpdatesDisabled && pendingCmuxSessionStatusWrites.size > 0
      ) {
  ...
  const enqueueNextBusyAnimationFrame = (sessionID: string): void => {
    ...
    const frameText = CMUX_BUSY_ANIMATION_FRAMES[frameIndex] ??
      CMUX_BUSY_ANIMATION_FRAMES[0];
  ...
  const startBusyAnimationTicker = (): void => {
    if (
      busyAnimationTicker || cmuxStatusUpdatesDisabled ||
  ...
  const applyCmuxSessionStatusTransition = (
    ...
    if (
      !notificationRuntime.preferCmux || !transition ||
