...
async function generateMetadata(
  ...
  const fallbackMetadata = (): GeneratedMetadata => {
    ...
    const title = firstLine.slice(0, 30).trim() +
      (firstLine.length > 30 ? "..." : "");
    const description = resultContent.slice(0, 150).trim() +
...
{"title": "Your Title Here", "description": "Your description here."}`;
    ...
    const textPart = responseParts?.find((p): p is TextPart =>
      p.type === "text"
    );
    if (!textPart) {
...
function createLogger(client: OpencodeClient) {
  const log = (level: "debug" | "info" | "warn" | "error", message: string) =>
    client.app.log({ body: { service: "background-agents", level, message } })
      .catch(() => {});
  return {
...
async function parseAgentMode(
  ...
  try {
    ...
    log.warn(
      `Agent list fetch failed for "${agentName}", assuming non-sub-agent: ${
...
async function parseAgentWriteCapability(
  ...
  try {
    ...
    // Fail-loud: Log for observability
    log.warn(
      `Config fetch failed for "${agentName}", assuming write-capable: ${
        error instanceof Error ? error.message : String(error)
      }`,
    );
...
class DelegationManager {
  ...
  constructor(
    ...
    this.readPollIntervalMs = options.readPollIntervalMs ??
      READ_POLL_INTERVAL_MS;
    this.terminalWaitGraceMs = options.terminalWaitGraceMs ??
  ...
  private hasUnreadCompletion(delegation: DelegationRecord): boolean {
    ...
    return delegation.retrieval.retrievedAt.getTime() <
  ...
  private buildTerminalNotification(
    ...
    const lines = [
      ...
      `<summary>Background agent ${delegation.status}: ${
  ...
  private buildDeterministicTerminalReadResponse(
    ...
    const lines = [
      ...
      `Started: ${
        delegation.startedAt?.toISOString() ||
        delegation.createdAt.toISOString()
      }`,
  ...
  private async notifyParent(delegationId: string): Promise<void> {
    try {
      ...
      await this.debugLog(
        `notifyParent failed for ${delegationId}: ${
          error instanceof Error ? error.message : "Unknown error"
        }`,
      );
    }
  ...
  private async getResult(delegation: DelegationRecord): Promise<string> {
    try {
      ...
      await this.debugLog(
        `getResult: Found ${messageData.length} messages. Roles: ${
          messageData.map((m) => m.info.role).join(", ")
        }`,
      );
      ...
      if (assistantMessages.length === 0) {
        await this.debugLog(
          `getResult: No assistant messages found in ${
            JSON.stringify(
              messageData.map((m) => ({
                role: m.info.role,
                keys: Object.keys(m),
      ...
      if (textParts.length === 0) {
        await this.debugLog(
          `getResult: No text parts found in message: ${
...
`;
      ...
      this.updateDelegation(delegation.id, (record) => {
        record.artifact.persistError = error instanceof Error
          ? error.message
  ...
  async readOutput(sessionID: string, id: string): Promise<string> {
    ...
    const immediateArtifactPath = delegation?.artifact.filePath ||
      fallbackFilePath;
...
Use \`delegation_read\` with the ID to retrieve full persisted output (including after compaction).`,
    ...
    async execute(args: DelegateArgs, toolCtx: ToolContext): Promise<string> {
      ...
      try {
        ...
        let response =
          `Delegation started: ${delegation.id}\nAgent: ${args.agent}`;
        if (totalActive > 1) {
          response += `\n\n${totalActive} delegations now active.`;
        }
        response += `\nYou WILL be notified when ${
          totalActive > 1 ? "ALL complete" : "complete"
        }. Do NOT poll.`;
...
Use this to retrieve results from delegated tasks if the inline notification was lost during compaction.`,
    args: {
      id: tool.schema.string().describe(
        "The delegation ID (e.g., 'elegant-blue-tiger')",
      ),
    },
...
function formatDelegationContext(
  ...
  if (running.length > 0) {
    ...
    for (const d of running) {
      ...
      if (d.prompt) {
        const truncatedPrompt = d.prompt.length > 200
  ...
  if (unreadCompleted.length > 0) {
    ...
    for (const d of unreadCompleted) {
      const statusEmoji = d.status === "complete"
        ? "✅"
        : d.status === "error"
        ? "❌"
        : d.status === "timeout"
...
const BackgroundAgentsPlugin: Plugin = async (ctx) => {
  ...
  return {
    ...
    "experimental.session.compacting": async (
      ...
      const unreadCompleted = manager.getUnreadCompletedDelegations(
        rootSessionID,
        10,
      ).map((d) => ({
        id: d.id,
    ...
    event: async ({ event }: { event: Event }): Promise<void> => {
      ...
      if (event.type === "message.updated") {
        ...
        if (sessionID) {
          const messageText = eventProperties.info.role === "assistant"
