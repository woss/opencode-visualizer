...
async function ensureLaunchContextExecutable(
  ...
  const resolveExecutable = options.resolveExecutable ??
    ((command: string) => Bun.which(command));
...
async function ensureLaunchContextProfile(
  launchContext: ActiveLaunchContext,
  validateProfileAvailability: ValidateProfileAvailability =
      ...
      client.app
        .log({
          body: {
            ...
            level: "error",
            message:
              `forkWithContext: Failed to clean up workspace dir ${destWorkspaceDir}: ${e}`,
          },
        })
        .catch(() => {});
    });
        ...
        client.app
          .log({
            body: {
              ...
              level: "error",
              message:
                `forkWithContext: Failed to clean up delegations dir ${destDelegationsDir}: ${e}`,
            },
          })
          .catch(() => {});
      },
  ...
  for (const command of commands) {
    ...
    try {
      ...
      if (result.exitCode !== 0) {
        ...
        log.warn(
          `[worktree] Hook failed (exit ${result.exitCode}): ${command}${
            stderr ? `\n${stderr}` : ""
          }`,
        );
      }
        ...
        async execute(args, toolCtx) {
          ...
          if (!branchResult.success) {
            return `❌ Invalid branch name: ${
              branchResult.error.issues[0]?.message
            }`;
          }
          ...
          if (args.baseBranch) {
            ...
            if (!baseResult.success) {
              return `❌ Invalid base branch name: ${
                baseResult.error.issues[0]?.message
              }`;
            }
          }
          ...
          const persistedLaunchMetadata = toPersistedLaunchMetadata(
            activeLaunchContext,
          );
          ...
          if (!terminalResult.success) {
            return `❌ Failed to launch worktree terminal: ${
              terminalResult.error ?? "unknown error"
...
const WorktreePluginWithInternals = Object.assign(
  WorktreePlugin,
  {
    testInternals: {
      isPathLikeCommand,
